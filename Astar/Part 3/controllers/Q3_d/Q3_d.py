...
class AStarGraph(object):
    def __init__(self):
        self.waypoints = {
            'A': {'coords': (-0.426, 0.426), 'danger': 1},
            'B': {'coords': (-0.213, 0.497), 'danger': 7},
            'C': {'coords': (-0.426, 0.213), 'danger': 3},
            'D': {'coords': (-0.497, 0), 'danger': 5},
            'E': {'coords': (-0.497, -0.213), 'danger': 5},
            'F': {'coords': (-0.426, -0.426), 'danger': 1},
            'G': {'coords': (-0.142, -0.426), 'danger': 1},
            'H': {'coords': (0.142, -0.426), 'danger': 1},
            'I': {'coords': (0.142, -0.142), 'danger': 3},
            'J': {'coords': (-0.142, -0.071), 'danger': 10},
            'K': {'coords': (-0.071, 0.071), 'danger': 10},
            'L': {'coords': (-0.142, 0.213), 'danger': 10},
            'M': {'coords': (0.142, 0.213), 'danger': 3},
            'N': {'coords': (0.284, 0.426), 'danger': 1},
            'O': {'coords': (0.426, 0.213), 'danger': 3},
            'P': {'coords': (0.497, 0), 'danger': 5},
            'Q': {'coords': (0.497, -0.284), 'danger': 5},
            'R': {'coords': (0.426, -0.497), 'danger': 7},
        }
        self.connections = {
            'A': ['B', 'C'],
            'B': ['A'],
    ...
    def heuristic(self, start, goal):
        #Use Chebyshev distance heuristic if we can move one square eitheradjacent or diagonal
        start_coords = self.waypoints[start]['coords']
        goal_coords = self.waypoints[goal]['coords']
        dx = abs(start_coords[0] - goal_coords[0])
        dy = abs(start_coords[1] - goal_coords[1])
        return max(dx, dy)
    ...
    def move_cost(self, a, b):
        avg_danger = (self.waypoints[a]['danger'] + self.waypoints[b]['danger']) / 2
        base_cost = math.sqrt((self.waypoints[b]['coords'][0] - self.waypoints[a]['coords'][0]) ** 2 + (self.waypoints[b]['coords'][1] - self.waypoints[a]['coords'][1]) ** 2)
        return base_cost + (avg_danger)
...
def question_1e(start, goal, route, graph):
    ...
    for i in range(len(route)):
        step = f"{route[i]}: ({graph.waypoints[route[i]]['coords'][0]}, {graph.waypoints[route[i]]['coords'][1]})"
        steps.append(step)
...
def question_1f(start, goal, route, graph):
    steps = []
    steps.append(f"Step {1}: ({start[0]}, {start[1]})-{graph.waypoints[route[0]]['coords']}")
    for i in range(len(route) - 1):
        step = f"Step {i+2}: {graph.waypoints[route[i]]}-{graph.waypoints[route[i+1]]['coords']}"
        steps.append(step)
    ...
    def run(self):
        ...
        while self.step(self.timestep) != -1:
           if not_done:
               ...
               first_waypoint = min(astar_graph.waypoints.keys(), key=lambda x: (start[0] - astar_graph.waypoints[x]['coords'][0])**2 + (start[1] - astar_graph.waypoints[x]['coords'][1])**2)
               last_waypoint = min(astar_graph.waypoints.keys(), key=lambda x: (goal[0] - astar_graph.waypoints[x]['coords'][0])**2 + (goal[1] - astar_graph.waypoints[x]['coords'][1])**2)
