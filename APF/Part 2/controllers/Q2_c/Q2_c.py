...
class Controller(Robot):
    ...
    def calculate_repulsion(self):
        #Angles of ech sensor in order wrt to the front of the robot
        sensor_orientations = [0.314, 0.785, 1.57, 2.62, -2.62, -1.57, -0.785, -0.314]
        # Simple repulsive force model based on proximity sensor readings
        repulsive_force_x = 0
        repulsive_force_y = 0
        for i, sensor in enumerate(self.proximity_sensors):
            reading = sensor.getValue()
            angle = sensor_orientations[i]
            #Sensor readings increase as robot gets closer to an object
            if (reading > 75):
