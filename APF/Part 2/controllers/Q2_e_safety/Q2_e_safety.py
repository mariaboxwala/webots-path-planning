...
class Controller(Robot):
    ...
    def calculate_repulsion(self):
        ...
        for i, sensor in enumerate(self.proximity_sensors):
            ...
            #Increased magntiudes for repulsive forces
            if (reading > 75):
                repulsive_force_x += math.cos(angle) * 5 
                repulsive_force_y += math.sin(angle)* 5
            if (reading > 100):
                repulsive_force_x += math.cos(angle) * 6
                repulsive_force_y += math.sin(angle)* 6
    ...
    def calculate_attraction(self, XR, YR):
        ...
        attraction_x = math.cos(theta) * 4
        attraction_y = math.sin(theta) * 4
    ...
    def calculate_motor_speeds(self, d, attract_force_x, attract_force_y, repulsive_force_x, repulsive_force_y):
        max_speed = 6.28  
        #slowed down speed of robot in general
        k_orientation = 0.3  # Proportional gain for orientation control
        k_distance = 0.3  # Proportional gain for distance control
        ...
        if repulsion_magnitude > repulsion_threshold:
            k_orientation += repulsion_magnitude *1.5 #Increased k_orientation for faster movement away from obstacles
        
        #Completely stop moving the robot when close enough to the goal
        at_goal_threshold = 0.01  # 1cm radius within the goal
        ...
        #Reduction of forward speed to further slow down robot to be able to react to obsatcles better
        forward_speed = force_magnitude/2 * k_distance 
