...
class Controller(Robot):
    ...
    def calculate_repulsion(self):
        #Changed sensor orientations
        sensor_orientations = [0.314,0,0,0,0,0,0, -0.314]
        repulsive_force_x = 0
        repulsive_force_y = 0
        for i, sensor in enumerate(self.proximity_sensors):
            ...
            if (reading > 75):
                    repulsive_force_x += math.cos(angle) * 2.5 
                    repulsive_force_y += math.sin(angle)* 2.5
            if (reading > 100):
    ...
    def calculate_motor_speeds(self, d, attract_force_x, attract_force_y, repulsive_force_x, repulsive_force_y):
        ...
        k_orientation = 2  # Proportional gain for orientation control
        k_distance = 0.5  # Proportional gain for distance control
        ...
        if repulsion_magnitude > repulsion_threshold:
            k_orientation += repulsion_magnitude 
        ...
        forward_speed = force_magnitude/2 * k_distance  # Reduce speed as distance decreases
